...
static meow_u8*
MeowHash_inv(meow_umm Len, void* SourceInit, void* buffer1)
{
    meow_u128 xmm0, xmm1, xmm2, xmm3, xmm4, xmm5, xmm6, xmm7; // NOTE(casey): xmm0-xmm7 are the hash accumulation lanes
    meow_u128 xmm8, xmm9, xmm10, xmm11, xmm12, xmm13, xmm14, xmm15; // NOTE(casey): xmm8-xmm15 hold values to be appended (residual, length)
    
    meow_u8* rcx;
    rcx = (meow_u8*)malloc(sizeof(meow_u8*) * 128);
    ...
    movdqu(xmm0, (meow_u8*)buffer1);
    movdqu(xmm1, (meow_u8*)buffer1);
    movdqu(xmm2, (meow_u8*)buffer1);
    movdqu(xmm3, (meow_u8*)buffer1);

    movdqu(xmm4, (meow_u8*)buffer1);
    movdqu(xmm5, (meow_u8*)buffer1);
    movdqu(xmm6, (meow_u8*)buffer1);
    movdqu(xmm7, (meow_u8*)buffer1);

    int unsigned LaneCount = (Len >> 5) & 0x7;
    if (LaneCount == 0) goto MixDown_inv; MEOW_MIX_inv(xmm2, xmm6, xmm0, xmm3, xmm4, rax + 0x00); --LaneCount;
    if (LaneCount == 0) goto MixDown_inv; MEOW_MIX_inv(xmm3, xmm7, xmm1, xmm4, xmm5, rax + 0x20); --LaneCount;
    if (LaneCount == 0) goto MixDown_inv; MEOW_MIX_inv(xmm4, xmm0, xmm2, xmm5, xmm6, rax + 0x40); --LaneCount;
    if (LaneCount == 0) goto MixDown_inv; MEOW_MIX_inv(xmm5, xmm1, xmm3, xmm6, xmm7, rax + 0x60); --LaneCount;
    if (LaneCount == 0) goto MixDown_inv; MEOW_MIX_inv(xmm6, xmm2, xmm4, xmm7, xmm0, rax + 0x80); --LaneCount;
    if (LaneCount == 0) goto MixDown_inv; MEOW_MIX_inv(xmm7, xmm3, xmm5, xmm0, xmm1, rax + 0xa0); --LaneCount;
    if (LaneCount == 0) goto MixDown_inv; MEOW_MIX_inv(xmm0, xmm4, xmm6, xmm1, xmm2, rax + 0xc0); --LaneCount;
con:
    pxor_clear(xmm9, xmm9);
    pxor_clear(xmm11, xmm11);
    ...
    movdqu_mem(rcx + 0x70, xmm7);
    movdqu_mem(rcx + 0x60, xmm6);
    movdqu_mem(rcx + 0x50, xmm5);
    movdqu_mem(rcx + 0x40, xmm4);

    movdqu_mem(rcx + 0x30, xmm3);
    movdqu_mem(rcx + 0x20, xmm2);
    movdqu_mem(rcx + 0x10, xmm1);
    movdqu_mem(rcx + 0x00, xmm0);
