...
static void
dump_buf(meow_u8* buf, unsigned int len)
{
    int i;
...
HashTestBuffer(void)
{
    ...
    char message[] = "Hao Wang 202000460078";
    printf("hash值为：%s\n", hash);
    printf("message为：%s\n", message);
    int Size = 23;
    //char* Buffer = (char*)malloc(Size);
    // NOTE(casey): Ask Meow for the hash
    static meow_u8 key1[128];
    meow_u128 Hash = MeowHash(MeowDefaultSeed, Size, message);
    meow_u8* key = MeowHash_inv(Size, message, hash);
    printf("秘钥key为：");
    dump_buf((meow_u8*)key, 128);
    for (int i = 0; i < 128; i++)
        key1[i] = key[i];
    meow_u128 Hash1 = MeowHash(key1, Size, message);
